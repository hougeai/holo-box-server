...
async def create_recharge(obj_in: RechargeCreate):
    ...
    if payment_method not in ['wechat', 'wechat-native', 'alipay']:
        return Fail(msg='支付方式只支持wechat/wechat-native/alipay')
    ...
    if payment_method == 'alipay':
        ...
        result = await payment_service.create_wx_payment(
            amount=amount, description='全息盒子', out_trade_no=trade_id, openid=user.openid, trade_type='JSAPI'
        )
        if result.get('success', False):
            obj = await Recharge.create(
                user_id=user_id,
    ...
    elif payment_method == 'wechat-native':
        # 微信扫码支付（网页端）
        # 生成商户订单号（必传）
        trade_id = datetime.now().strftime('%Y%m%d%H%M%S%f') + f'{random.randint(1000, 9999):04d}'
        # 创建微信支付订单
        result = await payment_service.create_wx_payment(
            amount=amount, description='全息盒子', out_trade_no=trade_id, trade_type='NATIVE'
        )
        if result.get('success', False):
            obj = await Recharge.create(
                user_id=user_id,
            ...
            # 返回二维码链接，前端生成二维码
            data = await obj.to_dict(exclude_fields=['id'])
            data['qr_code'] = result['data']['code_url']
            data['trade_id'] = trade_id
            return Success(data=data)
        else:
            return Fail(msg=result.get('error', ''))
...
async def retry_recharge(recharge_id: int = Body(..., embed=True, description='充值订单ID')):
    ...
    # 仅允许微信支付订单
    if recharge.payment_method not in ['wechat', 'wechat-native']:
...
async def get_recharge_status(trade_id: str = Query(..., description='交易ID')):
    ...
    if not obj.is_paid:
        ...
        elif obj.payment_method in ['wechat', 'wechat-native']:
        ...
        # 如果有退款单号，查询微信退款状态
        if obj.payment_method in ['wechat', 'wechat-native']:
