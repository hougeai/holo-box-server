...
from schemas.admin import RoleCreate, RoleUpdate, RoleUpdateMenusApis
...
async def list_role(
    ...
    total, role_objs = await role_controller.list(page=page, page_size=page_size, search=q, order=['id'])
...
async def create_role(role_in: RoleCreate):
    if await role_controller.is_exist(name=role_in.name):
        return Fail(code=400, msg='Role name already exists')
...
async def get_role_authorized(id: int = Query(..., description='角色ID')):
    ...
    # data = await role_obj.to_dict(m2m=True) # 这个地方需要修改
    auth_data = await role_obj.get_authorizations()
    # 获取角色基本信息
    data = await role_obj.to_dict()
    # 合并数据
    data.update({'menus': auth_data['menus'], 'apis': auth_data['apis']})
