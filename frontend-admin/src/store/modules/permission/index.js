...
function buildRoutes(routes = []) {
  return routes.map((e) => {
    const route = {
      name: e.name,
      path: e.path,
      component: shallowRef(Layout), // 使用布局组件
...
export const usePermissionStore = defineStore('permission', {
  ...
  getters: {
    routes() {
      return basicRoutes.concat(this.accessRoutes) // 基础路由 + 动态路由
    },
    menus() {
      return this.routes.filter((route) => route.name && !route.isHidden) // 过滤出有名字且不隐藏的路由作为菜单
    },
    apis() {
      return this.accessApis // 可访问的 API 列表
    },
  },
  actions: {
    // 这里最好加上，以便后续更改
