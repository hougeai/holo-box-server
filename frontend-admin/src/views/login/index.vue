    ...
    >
      ...
      <div w-320 flex-col px-20 py-35>
        <h5 f-c-c text-24 font-normal color="#6a6a6a">
          <icon-custom-logo mr-10 w-50 h-50 color-primary />{{ $t('app_name') }}
        </h5>
        <div mt-30>
          <n-input
...
const loading = ref(false) // 用于控制登录按钮的加载状态
async function handleLogin() {
  ...
  try {
    ...
    const res = await api.login({ email: username, password: password.toString(), remember: true })
    $message.success(t('views.login.message_login_success'))
    setToken(res.data.access_token)
    await addDynamicRoutes()
    // 当用户未登录时访问需要权限的页面，会被重定向到登录页，同时在 URL 中带上 redirect 参数，比如/login?redirect=/workbench
    if (query.redirect) {
      const path = query.redirect
      console.log('path', { path, query })
      Reflect.deleteProperty(query, 'redirect') // 删除 redirect 参数
      router.push({ path, query }) // 跳转到原来要访问的页面
    } else {
      router.push('/')
    }
  } catch (e) {
